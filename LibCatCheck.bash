...
# another event page.  The input file must be in LibComCat #
# format.  There is another script if the input is in      #
# ComCat format.                                           #
#                                                          #
# Input can be obtained by downloading using LibComCat     #
# search.  If the input was obtained using ComCat          #
# CatStat must be used                                     #
#                                                          # 
# Outputs:                                                 # 
...
do
	...
	NAU_OT=$(grep -w ${NA} ${file} | awk -F "\"*,\"*" '{print $2}')
	NAU_LT=$(grep -w ${NA} ${file} | awk -F "\"*,\"*" '{print $3}')
	NAU_LN=$(grep -w ${NA} ${file} | awk -F "\"*,\"*" '{print $4}')
	NAU_DP=$(grep -w ${NA} ${file} | awk -F "\"*,\"*" '{print $5}')
	NAU_MG=$(grep -w ${NA} ${file} | awk -F "\"*,\"*" '{print $6}')
...
do
	AU_OT=$(grep -w ${A} ${file} | awk -F "\"*,\"*" '{print $2}')
	AU_LT=$(grep -w ${A} ${file} | awk -F "\"*,\"*" '{print $3}')
	AU_LN=$(grep -w ${A} ${file} | awk -F "\"*,\"*" '{print $4}')
	AU_DP=$(grep -w ${A} ${file} | awk -F "\"*,\"*" '{print $5}')
	AU_MG=$(grep -w ${A} ${file} | awk -F "\"*,\"*" '{print $6}')
